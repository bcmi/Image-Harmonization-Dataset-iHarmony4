%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function extractBackgroundHistograms
...
%   of the image area to be considered. Store the marginal and pairwise 
%   histograms of the image background
% 
% Input parameters:
%
% Output parameters:
...
accObjects = [];
for c=1:length(colorSpaces)
    marginalVecBg{c} = zeros(nbObjects, nbBinsMarginal, 3, 'single');
    jointVecBg{c} = zeros(nbObjects, nbBinsJoint, nbBinsJoint, nbBinsJoint, 'single');
    accObjects{c} = 0;
end
...
fprintf('done!\n');
fprintf('Accumulated %d masks', accObjects{1});

save(fullfile(dbPath, 'backgroundsInfo.mat'), 'marginalVecBg', 'jointVecBg');
