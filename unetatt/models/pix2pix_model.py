...
class Pix2PixModel(BaseModel):
    ...
    def backward_D(self):
        ...
        # fake_AB = torch.cat((self.comp, self.harmonized), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
        fake_AB = self.harmonized # TODO: Added by Matt
        pred_fake = self.netD(fake_AB.detach())
        self.loss_D_fake = self.criterionGAN(pred_fake, False)
        # Real
        # real_AB = torch.cat((self.comp, self.real), 1)
        real_AB = self.real # TODO: Added by Matt
